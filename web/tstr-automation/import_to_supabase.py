...
def import_data():
    ...
    if not supabase_url or not supabase_key:
        print(
            "Error: Supabase URL or Key not found. Make sure they are in web/tstr-frontend/.env"
        )
        return
    ...
    try:
        ...
        if not categories_response.data:
            print(
                "No categories found in the database. Please seed the database first."
            )
            return
    ...
    try:
        with open(csv_file_path, mode="r", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                ...
                if not category_id:
                    print(
                        f"Warning: Category '{category_name}' not found for listing '{row.get('listing_title')}'. Skipping."
                    )
                    continue
                if not location_id:
                    print(
                        f"Warning: Location '{location_name}' not found for listing '{row.get('listing_title')}'. Skipping."
                    )
                    continue
                ...
                listing_data = {
                    ...
                    "source_script": "dual_scraper.py",
                    "script_location": "web/tstr-automation/",
                }
            ...
            if listings_to_insert:
                ...
                if len(insert_response.data) > 0:
                    print(
                        f"Successfully inserted {len(insert_response.data)} listings."
                    )
                else:
