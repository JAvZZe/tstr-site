...
export const GET: APIRoute = async ({ locals }) => {
  ...
  const supabaseUrl =
    env?.PUBLIC_SUPABASE_URL || import.meta.env.PUBLIC_SUPABASE_URL || 'FALLBACK_URL';

  const supabaseKey =
    env?.SUPABASE_SERVICE_ROLE_KEY || import.meta.env.SUPABASE_SERVICE_ROLE_KEY || 'FALLBACK_KEY';

  const debug = {
    urlSource: env?.PUBLIC_SUPABASE_URL
      ? 'runtime.env'
      : import.meta.env.PUBLIC_SUPABASE_URL
        ? 'import.meta.env'
        : 'fallback',
    keySource: env?.SUPABASE_SERVICE_ROLE_KEY
      ? 'runtime.env'
