...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!firstNameR.ok) return new Response(JSON.stringify({ error: firstNameR.error }), { status: 400 });
    if (!lastNameR.ok) return new Response(JSON.stringify({ error: lastNameR.error }), { status: 400 });
