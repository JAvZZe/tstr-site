...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Parse request data
    const { listingId, business_name, description, address, phone, email, website } =
      await request.json();
    ...
    console.log(
      `AUDIT: User ${user.id} (${user.email}) updated listing ${listingId} (${updatedListing.business_name}) at ${new Date().toISOString()}`
    );
